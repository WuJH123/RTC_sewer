...
constraints must still be differentiable when active.  Capacity bounds require
an explicit declaration that max-depth values are physical metres.
"""
...
def test_supported_depth_losses_reach_prediction_when_active():
    ...
    losses = PhysicsLosses(
        N,
        node_max_depth=torch.full((N,), 0.25),
        node_max_depth_is_physical_m=True,
    )(_pred(y_c, y_r))
...
def test_standardised_or_unverified_max_depth_does_not_activate_capacity():
    y_c = torch.ones(2, 4, 8) * 100.0
    y_r = torch.ones(2, 4, 8) * 100.0
    # Supplying a tensor is insufficient: callers must assert its physical unit.
    out = PhysicsLosses(8, node_max_depth=torch.ones(8))(_pred(y_c, y_r))
    assert out["capacity_bounds"].item() == pytest.approx(0.0)
