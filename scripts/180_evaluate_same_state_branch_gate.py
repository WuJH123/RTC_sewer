...
from sewerrtc.state.same_state_replay import evaluate_same_state_branch_gate


def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate formal same-state branch gate.")
    parser.add_argument("--config", required=True)
    parser.add_argument("--out-dir", default=str(OUT_ROOT / "state_clone"))
    args = parser.parse_args()
    del args.config
    code, outputs = evaluate_same_state_branch_gate(Path(args.out_dir))
