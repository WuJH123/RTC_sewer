...
def main() -> int:
    ...
    states = pd.read_parquet(args.state_manifest)
    ...
    def detail(path: str) -> pd.DataFrame:
        if path not in detail_cache:
            detail_cache[path] = pd.read_csv(path, low_memory=False)
    ...
    for index, candidate_row in candidates.iterrows():
        ...
        try:
            ...
            action_sha = str(candidate_row.get("candidate_action_sha256", "")).strip() or action_sha256(action)
