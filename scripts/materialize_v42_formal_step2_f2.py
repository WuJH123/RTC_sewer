...
def _resolve(completion:Path,payload:Mapping[str,Any],role:str)->Path:
 ...
 if not isinstance(branches,Mapping):raise KeyError(f'{completion}: completion.branches missing')
...
def _same_state(details:Mapping[str,pd.DataFrame],cp:float,nodes:list[str],fac:list[str])->bool:
 ref=_state_arrays(details['candidate'],cp,nodes,fac)
 for role in ('no_control','dynamic_internal','hold_previous'):
  cur=_state_arrays(details[role],cp,nodes,fac)
  if any(a.shape!=b.shape or not np.allclose(a,b,atol=1e-6,rtol=0.) for a,b in zip(ref,cur)):return False
 return True
def _state_sha(detail:pd.DataFrame,cp:float,nodes:list[str],fac:list[str])->str:
 h=hashlib.sha256()
 for a in _state_arrays(detail,cp,nodes,fac):h.update(np.ascontiguousarray(a,dtype=np.float64).tobytes(order='C'))
 return h.hexdigest()
...
def _engineering(payload:Mapping[str,Any],source:Mapping[str,Any],raw:bool)->bool:
 ...
 for names in ({'bounds','bounds_ok','bounds_pass'},{'rate','rate_ok','rate_pass'},{'ramp','ramp_ok','ramp_pass'},{'dwell','dwell_ok','dwell_pass'},{'interlock','interlock_ok','interlock_pass'}):
...
def _no_hotstart(payload:Mapping[str,Any],source:Mapping[str,Any])->bool:
 ...
 return False
def _physical(payload:Mapping[str,Any],source:Mapping[str,Any],frozen:str,raw:bool)->bool:
...
def main()->int:
 ...
 for row_i,(_,m) in enumerate(meta.iterrows(),start=1):
  ...
  try:
   ...
   future_rain=branches['candidate']['rainfall'];forcing=all(np.allclose(future_rain,branches[r]['rainfall'],atol=ATOL,rtol=0.) for r in ('no_control','dynamic_internal','hold_previous'));same=_same_state(details,float(cp),nodes,fac);state_sha=_state_sha(details['candidate'],float(cp),nodes,fac);ca=branches['candidate']['action'];ha=branches['hold_previous']['action'];changed=int(np.max(np.sum(np.abs(ca[:3]-ha[:3])>1e-8,axis=1)));semantics=all(_action_ok(branches[r]['action'],fac) for r in ROLES);authorized=bool(m.get('step2_accepted_from_manifest',False));raw=bool(m.get('raw_readmission_pending',False));engineering=_engineering(payload,src,raw);readback=all(branches[r]['action'].shape==(N_HORIZON_STEPS,N_FACILITIES) and np.isfinite(branches[r]['action']).all() for r in ROLES);pfv,tfv,peak=_kpis(branches,priority);kpi=all(math.isfinite(float(x)) for x in (pfv,tfv,peak))
   gates={'h120_eligible':True,'label_validity_pfv':math.isfinite(float(pfv)),'label_validity_tfv':math.isfinite(float(tfv)),'label_validity_peak':math.isfinite(float(peak)),'same_state_ok':same,'physical_sha_ok':_physical(payload,src,frozen,raw),'rainfall_sha_ok':bool(forcing and group),'prefix_sha_ok':bool(same and state_sha),'readback_ok':readback,'no_hotstart':_no_hotstart(payload,src) if raw else _gate(src,'no_hotstart'),'k_le_8':changed<=8,'actuator_semantics_ok':bool(semantics and engineering),'h120_window_complete':True,'kpi_recompute_ok':kpi}
   if authorized:
