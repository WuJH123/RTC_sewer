...
prepare -> Step1(3 seeds) -> causal GAT -> CONTROL_CORE/FULL_HYDRAULIC target
materialisation -> Step2(3 seeds) -> current-generation Calibration -> evidence.
...
def _status(root: Path) -> dict[str, Any]:
    """Summarise the current Formal training/calibration chain fail-closed."""
    formal = root / "outputs/project6_dual_reference_v4/final_v4/v42_paper/formal_f2"
    target_audit = _first_existing(
        formal / "step2/FORMAL_F2_STEP2_CONTROL_CORE_MANIFEST_TARGET_AUDIT.json",
        formal / "step2/FORMAL_F2_STEP2_FULL_HYDRAULIC_MANIFEST_TARGET_AUDIT.json",
    )
    paths = {
        ...
        "step2_calibration": formal / "calibration/PFV_ONLY_SAFETY_CALIBRATION.json",
    ...
    if payload.get("target_step2") is None or payload["target_step2"].get("status") != "pass":
        structural_reasons.append("formal_step2_target_contract_not_pass")
    if len(s2) < 3:
    ...
    if step2_reports:
        ...
        if any(
            x.get("step2_target_contract") not in {"CONTROL_CORE", "FULL_HYDRAULIC"}
            for x in step2_reports
        ):
            structural_reasons.append("step2_target_contract_missing_or_invalid")
        if any(
            x.get("storage_supervised") is not True
            or x.get("facility_flow_supervised") is not True
            for x in step2_reports
        ):
    ...
    step1_cal = payload.get("step1_calibration")
    if not step1_cal or step1_cal.get("status") != "pass":
        calibration_reasons.append("step1_current_calibration_uncertainty_ood_not_pass")
    step2_cal = payload.get("step2_calibration")
    if not step2_cal or step2_cal.get("status") != "pass":
        calibration_reasons.append("step2_current_calibration_pfv_budget_not_pass")
    elif step2_cal.get("safety_calibrated") is not True:
        calibration_reasons.append("step2_current_calibration_pfv_budget_not_calibrated")
    elif step2_cal.get("pfv_safety_statistic") != PFV_SAFETY_STATISTIC:
        calibration_reasons.append("step2_current_calibration_wrong_pfv_safety_statistic")
    elif int(step2_cal.get("pfv_predicted_safe_count", 0)) <= 0:
        calibration_reasons.append("step2_current_calibration_empty_admitted_set")
    ...
    payload["formal_training_chain_ready"] = bool(
        payload["structural_training_chain_pass"]
        and payload["calibration_chain_pass"]
        and payload["training_evidence_compiled"]
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--step1-pin-memory", action=argparse.BooleanOptionalAction, default=True
    )
    ...
    formal = root / "outputs/project6_dual_reference_v4/final_v4/v42_paper/formal_f2"
    prep = formal / "prepare"
    step1_root = formal / "step1"
    step2_root = formal / "step2"
    ...
    def step1() -> None:
        for seed in args.seeds:
            ...
            command = [
                ...
                "--persistent-workers"
                if args.step1_persistent_workers
                else "--no-persistent-workers",
            ]
            if args.step1_cache_dir is not None:
                command.extend(["--cache-dir", str(args.step1_cache_dir)])
    ...
    def step2() -> None:
        ...
        if args.step2_target_contract == "CONTROL_CORE":
            target_manifest = step2_root / "FORMAL_F2_STEP2_CONTROL_CORE_MANIFEST.parquet"
        else:
            target_manifest = step2_root / "FORMAL_F2_STEP2_FULL_HYDRAULIC_MANIFEST.parquet"
        _run(
            [
                py,
                "-u",
    ...
    if args.stage == "audit" and not payload["formal_training_chain_ready"]:
        return 3
    return 0
