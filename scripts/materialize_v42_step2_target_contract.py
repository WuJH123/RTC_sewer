...
invent or impute targets.  It re-reads each authoritative branch detail at the
12 exact H120 timestamps and appends the additional targets required by the
selected contract:
...
The script also makes the Project6 No-control semantics auditable: all 36 managed
facilities must be fully open/on (setting=1) throughout H120. Four-reference
action/hydraulic equivalence is recorded rather than silently treated as four
...
def _indices_by_static_flag(graph: dict[str, Any], flag: str) -> list[int]:
    cols = list(map(str, graph.get("node_static_cols", [])))
    if flag not in cols:
        raise KeyError(f"graph node_static has no {flag!r} column")
    idx = cols.index(flag)
    values = np.asarray(graph["node_static"], dtype=float)[:, idx]
    return [int(i) for i in np.flatnonzero(values > 0.5)]
...
def _exact_future(detail: pd.DataFrame, checkpoint: float) -> pd.DataFrame:
    ...
    for k in range(1, HORIZON_STEPS + 1):
        ...
        if len(found) != 1:
            raise ValueError(f"expected exactly one row at elapsed_min={target}, got {len(found)}")
...
def _extract(frame: pd.DataFrame, columns: list[str], *, required: bool) -> tuple[np.ndarray | None, float]:
    ...
    if required and fraction < 1.0 - 1.0e-12:
        raise ValueError(f"required target contains NaN/Inf, finite_fraction={fraction:.6f}")
    if fraction < 1.0 - 1.0e-12:
        return None, fraction
    return values.astype(np.float32), fraction


def _cached_detail(cache: OrderedDict[str, pd.DataFrame], path: Path, max_items: int) -> pd.DataFrame:
...
def main() -> int:
    ...
    ap.add_argument("--target-contract", choices=("CONTROL_CORE", "FULL_HYDRAULIC"), default="CONTROL_CORE")
    ...
    storage_idx = _indices_by_static_flag(graph, "is_storage")
    outfall_idx = _indices_by_static_flag(graph, "is_outfall")
    storage_ids = [node_ids[i] for i in storage_idx]
    outfall_ids = [node_ids[i] for i in outfall_idx]
    ...
    for row_number, (_, row) in enumerate(frame.iterrows(), start=1):
        ...
        try:
            ...
            actions = {b: _json_array(row[f"action_{b}_readback"]) for b in BRANCHES}
            depths = {b: _json_array(row[f"trajectory_depth_{b}"]) for b in BRANCHES}
            floods = {b: _json_array(row[f"trajectory_flood_{b}"]) for b in BRANCHES}
            if not no_control_all_open(actions["no_control"]):
                raise RuntimeError("No-control action contract violated: Engineering36 must be all-open/on (=1)")
            ...
            for branch in BRANCHES:
                ...
                storage, storage_fraction = _extract(future, storage_cols, required=True)
                facility, facility_fraction = _extract(future, facility_cols, required=True)
                outfall, outfall_fraction = _extract(future, outfall_cols, required=require_outfall)
                record[f"trajectory_storage_volume_{branch}"] = json.dumps(storage.tolist(), allow_nan=False)
                record[f"trajectory_facility_flow_{branch}"] = json.dumps(facility.tolist(), allow_nan=False)
                ...
                if outfall is not None:
                    record[f"trajectory_outfall_flow_{branch}"] = json.dumps(outfall.tolist(), allow_nan=False)
            ...
            equivalence = branch_equivalence(actions, depths=depths, floods=floods)
            for pair in equivalence["action_equivalent_pairs"]:
                equivalence_action_counts[pair] = equivalence_action_counts.get(pair, 0) + 1
            for pair in equivalence["hydraulic_equivalent_pairs"]:
                equivalence_hydraulic_counts[pair] = equivalence_hydraulic_counts.get(pair, 0) + 1
            ...
            record.update(
                {
                    ...
                    "full_hydraulic_target_coverage_complete": bool(branch_outfall_available),
                    ...
                    "reference_action_equivalent_pairs": json.dumps(equivalence["action_equivalent_pairs"]),
                    "reference_hydraulic_equivalent_pairs": json.dumps(equivalence["hydraulic_equivalent_pairs"]),
                    "reference_unique_action_branch_count": int(equivalence["unique_action_branch_count"]),
                    "reference_unique_hydraulic_branch_count": int(equivalence["unique_hydraulic_branch_count"] or 0),
                }
            )
    ...
    groups = int(out["split_group_key"].astype(str).nunique()) if not out.empty else 0
    ...
    audit_path = args.output_manifest.with_name(args.output_manifest.stem + "_TARGET_AUDIT.json")
