...
canonical PFV-first selector chooses one, and the selected outcome is then read
from recorded authoritative SWMM. Candidate prediction safety and fallback
actual safety are reported separately: a legal fallback is not silently counted
as a surrogate-predicted-safe candidate.
"""
...
from sewerrtc.control.pfvfirst_mpc_v42 import EngineeringStatus, FrozenFallback, MPCandidate, MPCWeights, SafetyMargins, decide_pfvfirst_mpc
...
def _branch_metrics(row: pd.Series, branch: str, priority_idx: list[int]) -> dict[str, float]:
    ...
    return {"pfv_m3": float(priority.sum() * DT_SEC), "tfv_m3": float(system.sum() * DT_SEC), "peak_m3s": float(system.max())}
...
def main() -> int:
    ...
    model = MultiReferenceHydraulicSurrogate(
        ...
        static_feature_dim=int(graph["node_static"].shape[1]), hidden_dim=int(cfg.get("hidden_dim", 32)),
        gat_heads=4, gat_layers=int(cfg.get("gat_layers", 2)), horizon=12,
    ).to(device)
    ...
    with torch.no_grad():
        for start in range(0, len(val_f), args.batch_size):
            ...
            out = _forward(model, _slice(data, idx), (edge_index, node_static, action_map), priority_tensor, device)
    ...
    for state_key, group in val_reset.groupby("state_key", sort=True):
        ...
        for i in indices:
            ...
            row_by_id[cid] = row
        fallback = FrozenFallback("hold_previous", hold_seq, "PROJECT6_V42_FAST_REPLAY_FALLBACK", True, {"development_only": True})
        decision = decide_pfvfirst_mpc(candidates=candidates, fallback=fallback, margins=SafetyMargins(max_changed_facilities=8), weights=MPCWeights(action=0.05, terminal=0.0, uncertainty=0.0))
        if decision.used_fallback:
            ...
            selected_pred = {"pfv": float(predicted["pfv_delta"][chosen_i]), "tfv": float(predicted["tfv_delta"][chosen_i]), "peak": float(predicted["peak_delta"][chosen_i])}
        ...
        candidate_predicted_safe = bool(not decision.used_fallback and selected_pred["pfv"] is not None and selected_pred["peak"] is not None and selected_pred["pfv"] <= 0.0 and selected_pred["peak"] <= 0.0)
        # Retain the legacy aggregate field for backward compatibility, but never
        # interpret fallback as a surrogate prediction in the new metrics.
        legacy_predicted_safe = bool(decision.used_fallback or candidate_predicted_safe)
        replay_rows.append(
            {
                "state_key": state_key, "candidate_count": len(indices), "selected_id": decision.selected_id,
                "used_fallback": decision.used_fallback, "predicted_pfv_delta_m3": selected_pred["pfv"],
                "predicted_tfv_delta_m3": selected_pred["tfv"], "predicted_peak_delta_m3s": selected_pred["peak"],
                "actual_pfv_delta_nc_m3": actual["pfv_delta_nc_m3"], "actual_tfv_delta_di_m3": actual["tfv_delta_di_m3"],
                "actual_peak_delta_di_m3s": actual["peak_delta_di_m3s"], "proposal_pfv_m3": actual["pfv_m3"],
                "proposal_tfv_m3": actual["tfv_m3"], "proposal_peak_m3s": actual["peak_m3s"],
                "no_control_pfv_m3": nc["pfv_m3"], "no_control_tfv_m3": nc["tfv_m3"], "no_control_peak_m3s": nc["peak_m3s"],
                "dynamic_internal_pfv_m3": di["pfv_m3"], "dynamic_internal_tfv_m3": di["tfv_m3"], "dynamic_internal_peak_m3s": di["peak_m3s"],
                "hold_previous_pfv_m3": hold["pfv_m3"], "hold_previous_tfv_m3": hold["tfv_m3"], "hold_previous_peak_m3s": hold["peak_m3s"],
                "candidate_predicted_safe": candidate_predicted_safe,
                "selected_candidate_false_safe": bool(candidate_predicted_safe and not actual_safe),
                "fallback_actual_safe": bool(actual_safe) if decision.used_fallback else None,
                "predicted_safe": legacy_predicted_safe,
                "actual_safe": actual_safe,
                "false_safe": bool(legacy_predicted_safe and not actual_safe),
            }
        )
    ...
    if replay_rows:
        with csv_path.open("w", newline="", encoding="utf-8") as f:
            ...
            writer.writeheader(); writer.writerows(replay_rows)
    ...
    summary = {
        ...
        "nonfallback_selection_rate": None if not replay_rows else float(len(selected_rows) / len(replay_rows)),
        ...
        "selected_candidate_false_safe_rate": _rate(selected_rows, "selected_candidate_false_safe"),
        "actual_safety_rate": _rate(replay_rows, "actual_safe"),
        "false_safe_rate": _rate(replay_rows, "false_safe"),
        "legacy_false_safe_note": "legacy false_safe treats fallback as predicted_safe; use selected_candidate_false_safe_rate for surrogate candidate safety",
    ...
    summary["go_signal"] = bool(
        ...
        and (summary["selected_candidate_false_safe_rate"] is None or summary["selected_candidate_false_safe_rate"] <= 0.2)
    )
