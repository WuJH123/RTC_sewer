"""Run the development-only V4.2 64+ rainfall end-to-end potential test.

This orchestrator is intentionally separate from the formal paper mainline.  It
uses enough independent rainfall groups to expose obvious generalisation/control
failures while keeping the experiment small enough for iterative debugging.

Execution order
---------------
1. choose >=64 (default 96) diverse Step1 auxiliary rainfall groups;
2. choose >=64 strict four-reference Step2 rainfall groups, preferring Train1600,
   with >=3 candidates at each state and a >=120 min checkpoint warm-up;
3. train/reuse a small temporal sparse GAT;
4. rebuild each Step2 history from thirteen causal GAT reconstructions;
5. replace realised future rainfall input with a causal decision-time forecast;
6. train the trajectory-first four-reference surrogate;
7. replay PFV-first selection and score the selected historical candidate with
   recorded authoritative SWMM;
8. compare Proposed with No-control, Internal, Hold, EFD, Auto-RBC and All-close.

All artifacts are stamped development-only and cannot authorize formal evidence.
"""
...
def main() -> int:
    ...
    selection, step2_raw = build_fast_step2_dataset_64plus(
        project_root=root,
