...
def _network_path_from_config(config_path: Path) -> Path | None:
    ...
    for raw in text.splitlines():
        line = raw.strip()
        if "wuhan_v8_storage_retrofit.inp" in line and ":" in line:
...
def _legacy_gat_lock_status(project_root: Path) -> tuple[str, str]:
    """Read historical sr0p15 evidence without upgrading it to formal Step1."""
    gat_dir = (
        project_root
        / "outputs"
    ...
    if gate_path.exists():
        try:
            robustness = str(
                json.loads(
                    gate_path.read_text(encoding="utf-8-sig")
                ).get("status")
                or "pending"
            )
        except Exception:
...
def build_augmented_state_contract_outputs(
    config_path: Path,
    gat_compatibility_path: Path,
    out_dir: Path,
) -> dict[str, Path]:
    """Write V4.2 sparse-state schema/audit templates without false admission.

    Historical sr0p15 lock/robustness evidence is retained as lineage only.  It
    cannot freeze the final temporal/action/link-static Step-1 model.
    """
    ...
    contract = build_state_feature_contract(
    ...
    legacy_lock, legacy_robustness = _legacy_gat_lock_status(
        config_path.parent.parent
    )
    contract["historical_selected_depth_gat"] = "sr0p15"
    contract["historical_selection_lock_status"] = legacy_lock
    contract["historical_gat_robustness_status"] = legacy_robustness
    contract["historical_gat_can_authorize_formal_step1"] = False
    contract["formal_reconstructor"] = FORMAL_RECONSTRUCTOR
    contract["formal_reconstructor_training_status"] = "pending"
    contract["formal_reconstructor_lock_status"] = "pending"
    ...
    paths = {
        ...
        "temporal_state_alignment_audit": out_dir
        / "temporal_state_alignment_audit.csv",
    ...
    paths["state_feature_contract"].write_text(
    ...
    schema = {
        ...
        "state_tensor_shape": {
            ...
            "historical_selection_lock_status": legacy_lock,
            "historical_gat_robustness_status": legacy_robustness,
            "formal_reconstructor": FORMAL_RECONSTRUCTOR,
            "formal_reconstructor_status": "pending_training_and_compatibility_audit",
        },
    }
    paths["state_feature_schema"].write_text(
        json.dumps(schema, indent=2), encoding="utf-8"
    )
    with paths["facility_state_schema"].open(
        "w", newline="", encoding="utf-8"
    ) as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["field", "required", "online_available", "notes"],
        )
        writer.writeheader()
            ...
            writer.writerow(
                {
                    "field": field,
                    "required": True,
                    "online_available": "depends_on_source",
                    "notes": "",
                }
            )
    with paths["temporal_state_alignment_audit"].open(
        "w", newline="", encoding="utf-8"
    ) as f:
        writer = csv.DictWriter(
            f,
            ...
            writer.writerow(
                {
                    ...
                    "valid_before_decision": "",
                    "interpolation_method": (
                        "causal_last_observation_carried_forward"
                    ),
        ...
        json.dumps(
            {
                ...
                "legacy_sr0p15_alone_is_not_formal_step1": True,
                "formal_temporal_gat_training_required": True,
